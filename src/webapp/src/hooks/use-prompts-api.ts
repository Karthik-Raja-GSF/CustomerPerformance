...
import type { Prompt, CreatePromptInput } from "@/types/prompts"
...
export function usePromptsApi() {
  ...
  const fetchPrompts = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof ApiError ? err.message : "Failed to fetch prompts"
      setError(message)
    } finally {
      setIsLoading(false)
    }
  ...
  const createPrompt = useCallback(async (input: CreatePromptInput): Promise<Prompt | null> => {
    setError(null)
    try {
      const response = await apiClient.post<ApiResponse<Prompt>>("/prompts", input)
  ...
  const deletePrompt = useCallback(async (id: string): Promise<void> => {
    ...
    try {
      ...
      const message = err instanceof ApiError ? err.message : "Failed to delete prompt"
      setError(message)
      throw err
    }
  }, [])

  const activatePrompt = useCallback(async (id: string): Promise<Prompt | null> => {
    setError(null)
    try {
      const response = await apiClient.patch<ApiResponse<Prompt>>(`/prompts/${id}/activate`)
      ...
      const message = err instanceof ApiError ? err.message : "Failed to activate prompt"
      setError(message)
      throw err
    }
  }, [])
