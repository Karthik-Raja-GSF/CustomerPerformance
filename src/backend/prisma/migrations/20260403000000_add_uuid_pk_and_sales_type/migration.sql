-- Step 1: Drop old FK first (depends on old PK)
ALTER TABLE "ait"."customer_bid_export_item"
  DROP CONSTRAINT "customer_bid_export_item_source_db_site_code_customer_bill_fkey";
DROP INDEX IF EXISTS "ait"."customer_bid_export_item_source_db_site_code_customer_bill__idx";

-- Step 2: Add new columns to customer_bid_data (nullable first for backfill)
ALTER TABLE "ait"."customer_bid_data" ADD COLUMN "sales_type" INTEGER NOT NULL DEFAULT 0;
ALTER TABLE "ait"."customer_bid_data" ADD COLUMN "id" UUID;

-- Step 3: Backfill UUIDs for all existing rows
UPDATE "ait"."customer_bid_data" SET "id" = gen_random_uuid() WHERE "id" IS NULL;

-- Step 4: Make id NOT NULL, drop old PK, set new PK
ALTER TABLE "ait"."customer_bid_data" ALTER COLUMN "id" SET NOT NULL;
ALTER TABLE "ait"."customer_bid_data" DROP CONSTRAINT "customer_bid_data_pkey";
ALTER TABLE "ait"."customer_bid_data" ADD CONSTRAINT "customer_bid_data_pkey" PRIMARY KEY ("id");

-- Step 5: Add 6-field unique constraint for upserts
CREATE UNIQUE INDEX "customer_bid_data_source_db_site_code_customer_bill_to_item_key"
  ON "ait"."customer_bid_data"("source_db", "site_code", "customer_bill_to", "item_no", "school_year", "sales_type");

-- Step 6: Add bid_id to export items, populate from old composite FK
ALTER TABLE "ait"."customer_bid_export_item" ADD COLUMN "bid_id" UUID;

UPDATE "ait"."customer_bid_export_item" ei
SET "bid_id" = cbd."id"
FROM "ait"."customer_bid_data" cbd
WHERE ei."source_db" = cbd."source_db"
  ...
  AND cbd."sales_type" = 0;

-- Step 7: Make bid_id NOT NULL, drop old composite columns, add new FK
ALTER TABLE "ait"."customer_bid_export_item" ALTER COLUMN "bid_id" SET NOT NULL;
ALTER TABLE "ait"."customer_bid_export_item" DROP COLUMN "source_db";
ALTER TABLE "ait"."customer_bid_export_item" DROP COLUMN "site_code";
ALTER TABLE "ait"."customer_bid_export_item" DROP COLUMN "customer_bill_to";
ALTER TABLE "ait"."customer_bid_export_item" DROP COLUMN "item_no";
ALTER TABLE "ait"."customer_bid_export_item" DROP COLUMN "school_year";

CREATE INDEX "customer_bid_export_item_bid_id_idx" ON "ait"."customer_bid_export_item"("bid_id");

ALTER TABLE "ait"."customer_bid_export_item"
  ADD CONSTRAINT "customer_bid_export_item_bid_id_fkey"
  FOREIGN KEY ("bid_id") REFERENCES "ait"."customer_bid_data"("id")
  ON DELETE RESTRICT ON UPDATE CASCADE;

-- Step 7: Updated sync function with sales_type support (COOP rows coexist with direct)
...
BEGIN
  ...
  FOR v_xmax IN
    ...
    INSERT INTO ait.customer_bid_data (
      id, source_db, site_code, customer_bill_to, item_no, school_year, sales_type,
    ...
    LEFT JOIN last_year_bids lyb
      ...
      AND cb.sales_type      = lyb.sales_type
    ...
    ON CONFLICT (source_db, site_code, customer_bill_to, item_no, school_year, sales_type)
    DO UPDATE SET
