-- Fix: the previous migration (20260410) placed an ORDER BY at the bottom of
-- Branch B inside a UNION ALL, which PostgreSQL parses as the ORDER BY for
-- the *entire* UNION — putting the inner "sub" alias out of scope. Error:
-- "missing FROM-clause entry for table 'sub'".
--
-- Fix: extract the DISTINCT-ON'd coop-only data into its own CTE per year
-- (cy_coop_only_bids / ly_coop_only_bids), then reference it as a simple
-- SELECT inside the UNION ALL. No stray ORDER BY inside the union.
--
-- This also makes the function easier to read — each CTE has a single job.
...
BEGIN
  ...
  FOR v_xmax IN
    WITH cy_coop_campaign_by_customer AS (
      -- One coop campaign per (source_db, customer_bill_to, item_no).
      -- Deterministic tiebreak: alphabetically lowest sales_code.
    ...
    cy_coop_only_bids AS (
      -- Coop-only customers for the current year: one row per
      -- (source_db, site_code, customer_bill_to, item_no). Excludes customers
      -- who also have a direct price for the same item in the same period.
      SELECT DISTINCT ON (sub.source_db, sub.site_code, sub.customer_bill_to, sub.item_no)
        sub.source_db,
        sub.site_code,
        sub.customer_bill_to,
        sub.item_no,
        sub.bid_qty,
        sub.bid_start,
        sub.bid_end,
      ...
      ORDER BY sub.source_db, sub.site_code, sub.customer_bill_to, sub.item_no, sub.com_co_op_code
    ),
    current_year_bids AS (
      -- Branch A: Direct customer bids (sales_type=0). LEFT JOIN to the
      -- coop lookup so com_co_op_code is populated when the customer ALSO
      -- happens to be in a coop for the same item.
      SELECT
        sp.source_db,
        c.location_code  AS site_code,
      ...
      -- Branch B: Coop-only customers, pre-deduplicated in cy_coop_only_bids.
      SELECT
        source_db,
        site_code,
        customer_bill_to,
        ...
        3 AS sales_type,
        com_co_op_code
      FROM cy_coop_only_bids
    ),
    ly_coop_only_bids AS (
      -- Coop-only customers for last year, deduped the same way. No need
      -- for bid_start/bid_end here — last_year_bids only feeds bid_qty and
      -- the is_new existence check.
      SELECT DISTINCT ON (sub.source_db, sub.site_code, sub.customer_bill_to, sub.item_no)
        sub.source_db,
        sub.site_code,
        sub.customer_bill_to,
    ...
    last_year_bids AS (
      ...
      -- Branch B: Coop-only last-year bids, pre-deduplicated.
      SELECT
        source_db,
        site_code,
        customer_bill_to,
