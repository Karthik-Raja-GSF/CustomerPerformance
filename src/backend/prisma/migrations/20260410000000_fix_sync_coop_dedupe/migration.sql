-- Fix: Branch B of current_year_bids and last_year_bids could emit multiple
-- rows per (source_db, site_code, customer_bill_to, item_no) when a customer
-- is a member of more than one coop that has a campaign price for the same
-- item — because the GROUP BY included sp.sales_code. That produced the
-- "ON CONFLICT DO UPDATE command cannot affect row a second time" (PG 21000)
-- error on the INSERT.
--
-- Fix: wrap Branch B in a DISTINCT ON subquery that picks one coop per
-- (source_db, site_code, customer_bill_to, item_no), ordered by sales_code
-- ascending (matching the cy_coop_campaign_by_customer tiebreak for
-- consistency across the function).
--
-- Everything else in the function is unchanged from migration 20260409.
...
BEGIN
  ...
  FOR v_xmax IN
    ...
    current_year_bids AS (
      ...
      -- Branch B: Coop-only customers. DISTINCT ON picks one coop per
      -- (source_db, site_code, customer_bill_to, item_no) so customers in
      -- multiple coops for the same item produce exactly one row.
    ...
    last_year_bids AS (
      ...
      -- Branch B: Coop-only last-year bids. Same DISTINCT ON dedupe as above.
      SELECT DISTINCT ON (sub.source_db, sub.site_code, sub.customer_bill_to, sub.item_no)
        sub.source_db,
        sub.site_code,
        sub.customer_bill_to,
